...
import { Scissors, CheckCircle2, MessageCircle, TrendingUp, Ticket, BarChart3 } from 'lucide-react';
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-block badge badge-brand mb-4">月額 ¥4,980 〜 / 初期費用 ¥0</div>
          <h1 className="text-5xl font-bold text-stone-900 leading-tight mb-6">
            ホットペッパーに頼らない<br />
            <span className="brand-text">自社集客</span>を、美容室に。
          </h1>
          <p className="text-lg text-stone-600 mb-8">
            LINE連携で予約・指名・カルテ・クーポンをワンストップ管理。<br />
            HPB→自社移行率を可視化し、広告費を月20万円→月5万円へ。
          </p>
          <div className="flex gap-3 justify-center">
            <Link href="/register" className="btn-brand px-6 py-3 text-base">
              <Scissors className="w-4 h-4" />新規登録（¥4,980/月〜）
            </Link>
          ...
          <div className="mt-4 text-xs text-stone-500">
            <Link href="/book/hair-salon-demo" className="underline">デモ店舗の予約画面を見る →</Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-6 bg-stone-50">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-center text-stone-600 mb-10">同じ機能を、美容室相場より安く。</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-stone-200">
                  ...
                  <th className="text-center py-3">指名管理</th>
                </tr>
              </thead>
              <tbody>
                <tr className="brand-light-bg font-semibold">
                  <td className="py-3"><span className="brand-text">HairSalonLink ⭐</span></td>
                  <td className="text-center">¥0</td>
                  <td className="text-center brand-text">¥4,980</td>
                ...
                <tr className="border-b border-stone-100">
                  <td className="py-3 text-stone-600">LiME</td>
                  <td className="text-center text-stone-600">¥0</td>
                  <td className="text-center text-stone-600">¥5,500〜</td>
                  <td className="text-center">✅</td>
                  <td className="text-center">✅</td>
                </tr>
                <tr className="border-b border-stone-100">
                  <td className="py-3 text-stone-600">BeautyMerit</td>
                  <td className="text-center text-stone-600">¥30,000</td>
                  <td className="text-center text-stone-600">¥9,800〜</td>
                  <td className="text-center">✅</td>
                  <td className="text-center">✅</td>
                </tr>
                <tr>
                  <td className="py-3 text-stone-600">リピッテBeauty</td>
      ...
      <section id="features" className="py-16 px-6">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-6">
            ...
            <Feature icon={<TrendingUp className="w-6 h-6" />} title="HPB→自社移行追跡" desc="HPB経由の新規客がどれだけ自社予約にリピートしたかを可視化。" />
            <Feature icon={<Ticket className="w-6 h-6" />} title="セグメント別クーポン" desc="休眠顧客・VIP・初回来店客など、セグメント別にLINEクーポン配信。" />
            <Feature icon={<BarChart3 className="w-6 h-6" />} title="薬剤履歴カルテ" desc="カラー・パーマ薬剤の履歴・アレルギー・ダメージレベルを一元管理。" />
            <Feature icon={<Scissors className="w-6 h-6" />} title="指名・スタイリスト階層" desc="Director / Top / Stylist / Junior の階層と指名料を自動計算。" />
            <Feature icon={<CheckCircle2 className="w-6 h-6" />} title="シンプルなUI" desc="機能過多の競合と違い、美容室特化のシンプル設計。" />
          </div>
        </div>
      </section>
      ...
      <section id="pricing" className="py-16 px-6 bg-stone-50">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-6">
            ...
            <PlanBox name="Standard" price="¥4,980" desc="個人〜小規模サロン向け" features={['顧客 500名', '予約無制限', 'LINE連携', '指名管理', 'カルテ', '分析']} recommended />
            <PlanBox name="Pro" price="¥9,980" desc="成長期のサロン向け" features={['顧客無制限', 'AI分析', 'スタイルギャラリー', '複数スタッフ管理', '優先サポート']} />
          </div>
        </div>
      </section>

      <footer className="py-8 px-6 border-t border-stone-200 text-center text-xs text-stone-500">
        © 2026 HairSalonLink. 美容室の自社集客を、すべての個人店に。
      </footer>
    </div>
  );
}
